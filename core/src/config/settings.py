...
class Settings(BaseSettings):
    ...
    whatsapp_bridge_url: str = Field(
        "http://localhost:8080", description="WhatsApp bridge HTTP URL"
    )
    whatsapp_bridge_ws_url: str = Field(
        "ws://localhost:8080/ws", description="WhatsApp bridge WebSocket URL"
    )

    # Security
    approved_directory: Path = Field(..., description="Base directory for projects")
    allowed_phones: Optional[List[str]] = Field(
        None, description="Allowed phone numbers (e.g. 5511999999999)"
    )
    allowed_users: Optional[List[int]] = Field(
        None, description="Allowed user IDs (derived from phone numbers)"
    )
    ...
    @classmethod
    def parse_phone_list(cls, v: Any) -> Optional[List[str]]:
        """Parse comma-separated phone number list."""
        if v is None:
            return None
        if isinstance(v, str):
    ...
    def telegram_token_str(self) -> str:
        """Compatibility shim — not used in WhatsApp mode."""
        return ""
